...
const onProdutoChange = (item) => {
  ...
  if (produto && !item.preco_unitario) {
...
<template>
  <form class="space-y-4" @submit.prevent="submit">
    <BaseInput
      :model-value="modelValue.fornecedor"
      label="Fornecedor"
      placeholder="Nome do fornecedor"
      :error="errors.fornecedor?.[0]"
      @update:model-value="modelValue.fornecedor = $event"
    />
    ...
    <div class="flex items-center justify-between pt-2 border-t border-slate-200">
      <span class="text-sm font-medium text-slate-600">Total da compra:</span>
      <span class="text-lg font-semibold text-indigo-600">{{ formatMoney(total) }}</span>
    </div>

    <div class="flex justify-end">
      <BaseButton type="submit" :loading="loading">Registrar compra</BaseButton>
    </div>
  </form>
</template>
